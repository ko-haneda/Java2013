...
public class MyPropertyDialog extends JDialog implements ActionListener,AdjustmentListener, FocusListener {
	...
	public void focusLost(FocusEvent e) {
		...
		if(e.getSource() == tf_fontSize){
			...
			for(int i = 0; i < 3; i++){
				sb_bgRGB[i].setValue(checkInput(Integer.parseInt(tf_bgRGB[i].getText()), 0, 255));
				sb_chRGB[i].setValue(checkInput(Integer.parseInt(tf_chRGB[i].getText()), 0, 255));
			}
			tf_bgColor.setBackground(createColor(tf_bgRGB));
			tf_chColor.setBackground(createColor(tf_chRGB));
		}
